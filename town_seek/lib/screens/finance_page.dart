...
class _FinancePageState extends State<FinancePage> {
  ...
  List<Shop> get _filteredShops {
    ...
    // Define Finance Categories
    const financeTags = ['ATM', 'Loan', 'Insurance', 'Investment', 'Tax', 'Consulting', 'Exchange'];

    final financeShops = allShops.where((shop) {
      final shopTags = shop.tags;
      final category = shop.category?.toLowerCase() ?? '';

      // Explicitly exclude other top-level categories
      if (['education', 'schools', 'entertainment', 'public services', 'publicservices', 'groceries', 'food', 'fashion', 'health', 'hospital', 'services', 'animals', 'religious', 'commercial'].contains(category)) {
          return false;
      }

      // Base filter
      bool isFinancehop = (category == 'finance') ||
          shopTags.any((t) => financeTags.contains(t));

      return isFinancehop;
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return financeShops;

    final filtered = financeShops.where((shop) {
                    ...
                    if (_filteredShops.isEmpty)
                      const Center(
                        child: Padding(
                          ...
                          child: Text("No Finance match filters"),
                        ),
                      )
                    else
