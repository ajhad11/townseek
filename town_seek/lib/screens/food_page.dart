...
class _FoodPageState extends State<FoodPage> {
  ...
  List<Shop> get _filteredShops {
    ...
    // Filter ONLY Food related shops for this page
    final foodShops = allShops.where((shop) {
       final category = shop.category?.toLowerCase() ?? '';
       
       // Explicitly exclude other top-level categories
       if (['education', 'schools', 'entertainment', 'public services', 'publicservices', 'groceries', 'fashion', 'health', 'hospital', 'services', 'finance', 'animals', 'religious', 'commercial'].contains(category)) {
          return false;
       }

       return (category == 'food') ||
              shop.tags.any((tag) => 
         ['Food', 'Dining', 'Coffee', 'Restaurant', 'Cafe', 'Fast Food', 'Healthy'].contains(tag));
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return foodShops;

    final filtered = foodShops.where((shop) {
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 10),
                          _buildFilterChip('Take Away'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Home Delivery'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Veg'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Non Veg'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Meals'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Cool Bar'),
                          const SizedBox(width: 10),
                    ...
                    if (_filteredShops.isEmpty)
                      const Center(
                        child: Padding(
                          ...
                          child: Text("No shops match filters"),
                        ),
                      )
                    else
