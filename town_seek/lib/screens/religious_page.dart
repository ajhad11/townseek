...
class ReligiousPage extends StatefulWidget {
  ...
  @override
  State<ReligiousPage> createState() => _ReligiousPageState();
}
...
class _ReligiousPageState extends State<ReligiousPage> {
  ...
  List<Shop> get _filteredShops {
    ...
    // Define Religious Categories
    const religiousTags = ['Temple', 'Church', 'Mosque', 'Monastery', 'Shrine'];

    final religiousShops = allShops.where((shop) {
      final shopTags = shop.tags;
      final category = shop.category?.toLowerCase().replaceAll(' ', '') ?? '';

      // Explicitly exclude other top-level categories
      if (['education', 'schools', 'entertainment', 'publicservices', 'groceries', 'food', 'fashion', 'health', 'hospital', 'finance', 'services', 'animals', 'commercial'].contains(category)) {
          return false;
      }

      // Base filter
      bool isReligioushop = (category == 'religious') ||
          shopTags.any((t) => religiousTags.contains(t));

      return isReligioushop;
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return religiousShops;

    final filtered = religiousShops.where((shop) {
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 10),
                          _buildFilterChip('Temple'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Church'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Mosque'),
                          const SizedBox(width: 10),
                    ...
                    if (_filteredShops.isEmpty)
                      const Center(
                        child: Padding(
                          ...
                          child: Text("No Religious match filters"),
                        ),
                      )
                    else
