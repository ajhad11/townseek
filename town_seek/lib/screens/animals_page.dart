...
class _AnimalsPageState extends State<AnimalsPage> {
  ...
  List<Shop> get _filteredShops {
    ...
    // Define Animals Categories
    const animalsTags = ['Pet Shop', 'Veterinary', 'Aquarium', 'Kennel', 'Grooming', 'Animal Shelter'];

    final animalShops = allShops.where((shop) {
      final shopTags = shop.tags;
      final category = shop.category?.toLowerCase() ?? '';

      // Explicitly exclude other top-level categories
      if (['education', 'schools', 'entertainment', 'public services', 'publicservices', 'groceries', 'food', 'fashion', 'health', 'hospital', 'finance', 'services', 'religious', 'commercial'].contains(category)) {
          return false;
      }

      // Base filter
      bool isAnimalshop = (category == 'animals') ||
          shopTags.any((t) => animalsTags.contains(t));

      return isAnimalshop;
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return animalShops;

    final filtered = animalShops.where((shop) {
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 10),
                          _buildFilterChip('Pet Shop'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Veterinary'),
                          const SizedBox(width: 10),
                    ...
                    if (_filteredShops.isEmpty)
                      const Center(
                        child: Padding(
                          ...
                          child: Text("No Animals match filters"),
                        ),
                      )
                    else
