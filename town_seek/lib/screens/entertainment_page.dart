...
class _EntertainmentPageState extends State<EntertainmentPage> {
  ...
  List<Shop> get _filteredShops {
    ...
    // We ONLY want Service related shops for this page by default?
    // Or we let the filters drive it.
    // The user wants "Tailoring, rental shops, rooms, lodges, repairing shops" in this page.
    // We should probably filter the BASE list to only include these categories if "All" is selected?
    // Or just show everything that matches the specific service filters available.
    // Let's rely on the chips. "All" might show all Entertainment.
    
    // Define Entertainment Categories
    const entertainmentTags = ['Cinema', 'Gaming', 'Amusement Park', 'Bowling', 'Theater', 'Sports', 'Events', 'Music'];

    final entertainmentShops = allShops.where((shop) {
      final shopTags = shop.tags;
      final category = shop.category?.toLowerCase();

      // Explicitly exclude schools/education from entertainment even if they have overlapping tags
      if (category == 'education' || category == 'schools') return false;

      // Base filter
      bool isEntertainmenthop = (category == 'entertainment') ||
          shopTags.any((t) => entertainmentTags.contains(t));

      return isEntertainmenthop;
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return entertainmentShops;

    final filtered = entertainmentShops.where((shop) {
                  ...
                  children: [
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 10),
                          _buildFilterChip('Cinema'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Gaming'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Theater'),
                          const SizedBox(width: 10),
                    ...
                    if (_filteredShops.isEmpty)
                      const Padding(
                        ...
                        child:  Center(child: Text("No Entertainment match filters")),
                      ),
