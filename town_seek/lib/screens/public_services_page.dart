...
class _PublicServicesPageState extends State<PublicServicesPage> {
  ...
  List<Shop> get _filteredShops {
    ...
    // Define Public Services Categories
    const publicServicesTags = ['Government Office', 'Post Office', 'Police Station', 'Fire Station', 'Library', 'Community Center'];

    final publicServicesShops = allShops.where((shop) {
      final shopTags = shop.tags;
      final category = shop.category?.toLowerCase().replaceAll(' ', '') ?? '';

      // Explicitly exclude other top-level categories
      if (['education', 'schools', 'entertainment', 'groceries', 'food', 'fashion', 'health', 'hospital', 'services', 'finance', 'animals', 'religious', 'commercial'].contains(category)) {
          return false;
      }

      // Base filter
      bool isPublicServicesShop = (category == 'publicservices') ||
          shopTags.any((t) => publicServicesTags.contains(t));

      return isPublicServicesShop;
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return publicServicesShops;

    final filtered = publicServicesShops.where((shop) {
                    ...
                    if (_filteredShops.isEmpty)
                      const Center(
                        child: Padding(
                          ...
                          child: Text("No Public Services match filters"),
                        ),
                      )
                    else
