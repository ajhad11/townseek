...
class _EducationPageState extends State<EducationPage> {
  ...
  List<Shop> get _filteredShops {
    ...
    const educationTags = ['School', 'Schools', 'College', 'Kindergarten', 'University', 'Tutoring', 'Courses', 'Language', 'Library'];

    final educationShops = allShops.where((shop) {
      final shopTags = shop.tags;
      final category = shop.category?.toLowerCase();

      // Explicitly exclude entertainment from schools even if they have overlapping tags like "Sports"
      if (category == 'entertainment') return false;

      // Base filter
      bool isEducationhop = (category == 'education') ||
          (category == 'schools') ||
          shopTags.any((t) => educationTags.contains(t));

      return isEducationhop;
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return educationShops;

    final filtered = educationShops.where((shop) {
                    ...
                    if (_filteredShops.isEmpty)
                      const Center(
                        child: Padding(
                          ...
                          child: Text("No Education match filters"),
                        ),
                      )
                    else
