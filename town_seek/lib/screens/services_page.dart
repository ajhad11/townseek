...
class _ServicesPageState extends State<ServicesPage> {
  ...
  List<Shop> get _filteredShops {
    ...
    const serviceTags = ['Tailor', 'Rental', 'Lodge', 'Repair', 'Service'];

    final serviceShops = allShops.where((shop) {
      final shopTags = shop.tags;
      final category = shop.category?.toLowerCase().replaceAll(' ', '') ?? '';

      // Explicitly exclude other top-level categories
      if (['education', 'schools', 'entertainment', 'publicservices', 'groceries', 'food', 'fashion', 'health', 'hospital', 'finance', 'animals', 'religious', 'commercial'].contains(category)) {
          return false;
      }

      // Base filter: Must likely be a service shop?
      bool isServiceShop = (category == 'services') ||
          shopTags.any((t) => serviceTags.contains(t) || 
          ['Stitching', 'Rooms', 'Mobile', 'Laptop', 'Tuxedo'].contains(t)); // Extended heuristic

      return isServiceShop;
    }).toList();

    if (_selectedFilters.isEmpty || _selectedFilters.contains('All')) return serviceShops;

    final filtered = serviceShops.where((shop) {
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 10),
                          _buildFilterChip('Tailor'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Rental'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Lodge'),
                          const SizedBox(width: 10),
                          _buildFilterChip('Repair'),
                          const SizedBox(width: 10),
                    ...
                    if (_filteredShops.isEmpty)
                      const Center(
                        child: Padding(
                          ...
                          child: Text("No services match filters"),
                        ),
                      )
                    else
